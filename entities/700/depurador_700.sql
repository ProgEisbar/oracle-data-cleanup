...
--  EJEMPLO para la ejecución: EXEC SOPORTEDBA.PRC_WFGADQ_ENTITY_DATA_CLEANING_1('PROD_ENTIDAD700','31/12/2021');
IS
BEGIN
...
BEGIN
    ...
    declare
        ...
        open CUR_ID_CONSUMOS;
            ...
            --
            -- EXECUTE IMMEDIATE 'COMMIT';
            -- CODIGO AJUSTADO SEGUN DEPURADOR 703 - 23/07/2026
            -- TC33A se elimina en el bloque principal de CONSUMOS Y TC33A,
            -- donde se conservan ID_CONSUMO e ID_TC33A antes de borrar CONSUMOS.
                    
        close CUR_ID_CONSUMOS;
    END;
END; 

--AGREGADO POR BORRADO DE CUOTA_CUOTA
--AGREGADO SE HACE EL DELETE PRIMERO EN AUTORIOZACION, PORQUE SI BORRO LOS CONSUMOS PRIMERO NO ME BORRA LA TABALA AUTORIZACION
-- DELETE DE LA TABLA AUTORIZACION CUOTA_CUOTA
        v_time2 := systimestamp;
        
         EXECUTE IMMEDIATE 'DELETE FROM ' || v_sche_ori || '.AUTORIZACION D WHERE D.ID_AUTORIZACION IN (SELECT ID_AUTORIZACION FROM PROD_ENTIDAD700.CONSUMOS WHERE ID_GRUPO_TRANSACCION IN (11,12) AND NRO_CUOTA = CUOTAS AND FECHA_PRESENT <= ' || v_date_dep || ')';
...
-- DELETE DE LAS TABLAS PROMOCION_LOG, CONSUMOS_DATOS_ADICIONALES, CONSUMOS_CARGOS_ADICIONALES, CONSUMOS_CUOTAS, CONSUMOS
...
BEGIN
    ...
    declare
         ...
         CURSOR CUR_ID_CONSUMOS
         is
       --  SELECT /*+ PARALLEL (2) */ ID_CONSUMO FROM CONSUMOS WHERE ID_GRUPO_TRANSACCION IN (11,12) AND NRO_CUOTA = CUOTAS AND FECHA_PRESENT <= v_fecha_dep; 
		 
		 SELECT /*+ PARALLEL (2) */ ID_CONSUMO FROM CONSUMOS WHERE TRUNC(FECHA_OPER) <= v_fecha_dep AND ID_GRUPO_TRANSACCION IN (11,12) AND NRO_CUOTA = CUOTAS AND FECHA_PRESENT <= v_fecha_dep; 
...
-- DELETE DE CONSUMOS
            DELETE FROM CONSUMOS WHERE ID_CONSUMO = registros(i);
...
-- DELETE DE LA TABLA COMERCIOS_LIQ_DETALLES:
      ...
      --NUEVO CODIGO SABRINA 1/10/2025  
...
-- DELETE DE LA TABLA CONSUMOS:
        ...
        -- v_time2 := systimestamp;
        --
        -- NUEVO CODIGO SABRINA 1/10/2025 con filtro para cuotas AND C.ID_GRUPO_TRANSACCION NOT IN (11,12)
...
-- CODIGO AJUSTADO SEGUN DEPURADOR 703 - NUEVA RELACION TC33A/CONSUMOS - 23/07/2026
-- Se guardan ambas claves antes del borrado. Se conserva el filtro de cuota a cuota de 700.
-- DELETE DE LAS TABLAS CONSUMOS Y TC33A:

        v_time2 := systimestamp;

        BEGIN
            DECLARE
                CURSOR CUR_ID_CONSUMOS_TC33A IS
                    SELECT /*+ PARALLEL (2) */ C.ID_CONSUMO, C.ID_TC33A
                      FROM CONSUMOS C
                     WHERE TRUNC(C.FECHA_OPER) <= v_fecha_dep
                       AND C.ID_GRUPO_TRANSACCION NOT IN (11,12);

                TYPE t_registro_ids IS RECORD (
                    id_con CONSUMOS.ID_CONSUMO%TYPE,
                    id_tc  CONSUMOS.ID_TC33A%TYPE);

                TYPE lv_tbl_ids IS TABLE OF t_registro_ids;
                registros_ids lv_tbl_ids;
            BEGIN
                OPEN CUR_ID_CONSUMOS_TC33A;
                FETCH CUR_ID_CONSUMOS_TC33A BULK COLLECT INTO registros_ids;

                FORALL i IN registros_ids.FIRST .. registros_ids.LAST
                    DELETE FROM CONSUMOS
                     WHERE ID_CONSUMO = registros_ids(i).id_con;
                ...
                FORALL i IN registros_ids.FIRST .. registros_ids.LAST
                    DELETE FROM TC33A
                     WHERE ID_TC33A = registros_ids(i).id_tc;
...
--  fecha_depuracion: fecha de depuración en formato DD/MM/YYYY
--  proc_id:id del proceso (cuando se ejecuta de manera automatica)
--  EJEMPLO para la ejecución: EXEC SOPORTEDBA.PRC_WFGADQ_ENTITY_DATA_CLEANING_2('PROD_ENTIDAD701','31/12/2021');
IS
BEGIN
        EXECUTE IMMEDIATE 'alter session set current_schema = SOPORTEDBA';
        EXECUTE IMMEDIATE 'alter session set nls_Date_format=''DD/MM/RRRR''';
...
-- UPDATE DE CONSUMOS (ARN) DE LA TABLA IPM:
        ...
        --v_time2 := systimestamp;
		--
        --EXECUTE IMMEDIATE 'UPDATE ' || v_sche_ori || '.CONSUMOS C SET ARN = (SELECT /*+ PARALLEL (2) */ DATOS_REFERENCIA_ADQUIRIENTE FROM ' || v_sche_ori || '.IPM I WHERE I.ID_CONSUMO = C.ID_CONSUMO AND ID_ESTADO = 0 AND ROWNUM = 1)
        --                   WHERE ID_CONSUMO IN (SELECT /*+ PARALLEL (2) */ ID_CONSUMO FROM ' || v_sche_ori || '.IPM WHERE ID_IPM_FILE IN (SELECT /*+ PARALLEL (2) */ ID_IPM_FILE FROM ' || v_sche_ori || '.IPM_FILE
...
-- UPDATE DE CONSUMOS (ARN) DE LA TABLA CTF_VISA:
        ...
        --v_time2 := systimestamp;
		--
        --EXECUTE IMMEDIATE 'UPDATE ' || v_sche_ori || '.CONSUMOS C SET ARN = (SELECT /*+ PARALLEL (2) */ DATOS_REFERENCIA_ADQUIRENTE FROM ' || v_sche_ori || '.CTF_VISA I WHERE I.ID_CONSUMO = C.ID_CONSUMO AND TCR = 0 AND ROWNUM = 1)
        --                  WHERE ID_CONSUMO IN (SELECT /*+ PARALLEL (2) */ ID_CONSUMO FROM ' || v_sche_ori || '.CTF_VISA WHERE ID_CTF_FILE IN (SELECT /*+ PARALLEL (2) */ ID_CTF_FILE FROM ' || v_sche_ori || '.CTF_FILE_VISA
...
--AGREGADO POR CUOTA CUOTA REGISTROS QUE YA CUMPLIERON SU CICLO.

--DELETE DE LA TABLA IPM CUOTA_CUOTA
...
--DELETE DE LA TABLA CTF_VISA CUOTA_CUOTA        
		v_time2 := systimestamp;
...
CREATE OR REPLACE PACKAGE BODY SOPORTEDBA.PKG_WFGADQ_ENTITY_DATA_COPY_AND_CLEANING_700 AS 
...
BEGIN
    ...
    DECLARE
        ...
        v_num_row 	    NUMBER;                                             -- NUMERO DE REGISTROS AFECTADOS EN LA SENTENCIA
		v_sche_ori 	    VARCHAR2 (20) := 'PROD_ENTIDAD700';                 -- ESQUEMA A ORIGEN
    ...
    BEGIN
       ...
       -- SE DEFINE LA FECHA DE LA DATA QUE VA A QUEDAR EN LINEA (6 MESES):
        ...
        v_date_dep3:= '''' || v_fecha_dep3 || '''';
        -- SE CALCULA LA FECHA INICIAL DE CADA MES NIVELACION:
        EXECUTE IMMEDIATE 'SELECT ADD_MONTHS(TRUNC(TO_DATE(''' || v_fecha_dep || '''),''MM''),-18) FROM DUAL' INTO v_fecha_inim;
...
BEGIN
    ...
    IF v_hora_actual < '02:59' OR v_hora_actual >= '03:50' THEN
...
-- IMPORT DE LAS TABLAS PROMOCION_LOG, CONSUMOS_DATOS_ADICIONALES, CONSUMOS_CARGOS_ADICIONALES, CONSUMOS_CUOTAS, RETENCIONES_CONSUMOS, PRESENTACIONES:
        ...
        -- CODIGO AJUSTADO SEGUN DEPURADOR 703 - NUEVA RELACION TC33A/CONSUMOS - 23/07/2026
        -- Se excluye TC33A del recorrido generico porque ya no se relaciona por ID_CONSUMO.
        FOR r IN (SELECT owner, table_name FROM DBA_TABLES WHERE owner = v_sche_dest
              AND table_name IN ('PROMOCION_LOG','CONSUMOS_DATOS_ADICIONALES','CONSUMOS_CARGOS_ADICIONALES','CONSUMOS_CUOTAS','RETENCIONES_CONSUMOS','PRESENTACIONES'))
        LOOP
            
            v_time2 := systimestamp;
            --SE AGREGA EL FILTRO --AND C.ID_GRUPO_TRANSACCION NOT IN (11,12));1/10/2025 SABRINA
...
-- IMPORT DE LA TABLA COMERCIOS_LIQ_DETALLES:
        ...
        ----CODIGO SABRINA BORRADO CUOTAS 1/10/2025
        EXECUTE IMMEDIATE 'INSERT /*+ APPEND */ INTO ' || v_sche_dest || '.COMERCIOS_LIQ_DETALLES SELECT /*+ PARALLEL (2) */ D.* FROM COMERCIOS_LIQ_DETALLES'|| TO_NUMBER(SUBSTR(v_sche_ori,13,3)) || ' D 
...
-- IMPORT DE LA TABLA COMERCIOS_LIQ_CABECERA:
        ...
        INSERT INTO SOPORTEDBA.LOG_DEPURADORES (PROCESS_ID,DATE_TIME,EXECUTION_TIME,CLEANING_DATE,ENTITY,MESSAGE,SP_NAME,ERROR_CODE,ERROR_BACKTRACE,STACK_ERROR) VALUES menor a ' || v_fecha_dep || ' numero de registros insertados en ' || v_sche_dest || '.' || 'AUTORIZ_INTENTOS_REVERSOS: ' || v_num_row, 'PRC_WFGADQ_ENTITY_DATA_COPY_AND_CLEANING_1',NULL,NULL,N v_num_row := SQL%rowcount;
        
        v_time3 := systimestamp;
            
        v_time_tot2:= substr(TO_CHAR(v_time3-v_time2,'SSSS.FF'),9,8);
                            
     FROM AUTORIZACION' || TO_NUMBER(SUBSTR(v_sche_ori,13,3)) || ' WHERE TRUNC(FECHA_AUTORIZACION) <= ' || v_date_dep ||
						 -- ' AND ID_AUTORIZACION NOT IN (SELECT /*+ PARALLEL (2) */ ID_AUTO' || v_date_ini3 || ' AND ' || v_date_dep6 || ' AND ID_AUTORIZACION_ORIGINAL IS NOT NULL)';
...
-- IMPORT DE LA TABLA AUTORIZACION_ADQUIRENTE_LOG:
        ...
        v_time_tot2:= substr(TO_CHAR(v_time3-v_time2,'SSS
-- IMPORT DE LA TABLA RESPUESTA_MC_LOG:

        v_time2 := systimestamp;

        EXECUTE IMMEDIATE 'INSERT /*+ APPEND */ INTO ' || v_sche_dest || '.RESPUESTA_MC_LOG SELECT /*+ PARALLEL HAR(v_time3-v_time2,'SSSS.FF'),9,8);
		
            INSERT INTO SOPORTEDBA.LOG_DEPURADORES (PROCESS_ID,DATE_TIME,EXECUTION_TIME,CLEANING_DATE,ENTITY,MESSAGE,SP_NAME,ERROR_CODE,ERROR_BACKTRAC    v_num_row := SQL%rowcount;
...
--AGREGADO       
--IMPORT DE LAS TABLAS PROMOCION_LOG,CONSUMOS_DATOS_ADICIONALES,CONSUMOS_CARGOS_ADICIONALES,CONSUMOS_CUOTAS,CONSUMOS POR CUOTA_CUOTA:
...
-- IMPORT DE LA TABLA AUTORIZACION CUOTA_CUOTA

        v_time2 := systimestamp;
        
        --INSERT REGISTROS DE CUOTA CUOTA QUE YA CUMPLIERON SU CICLO 
        EXECUTE IMMEDIATE 'INSERT /*+ APPEND */ INTO ' || v_sche_dest || '.AUTORIZACION SELECT /*+ PARALLEL (2) */ * FROM AUTORIZACION' || TO_NUMBER(SUBSTR(v_sche_ori,13,3)) || ' WHERE ID_AUTORIZACION IN (SELECT ID_AUTORIZACION FROM CONSUMOS' || TO_NUMBER(SUBSTR(v_sche_ori,13,3)) || ' WHERE ID_GRUPO_TRANSACCION IN (11,12) AND NRO_CUOTA = CUOTAS AND FECHA_PRESENT <= ' || v_date_dep || ')';
...
-------------------------------------------------------------
        ...
        SOPORTEDBA.PRC_WFGADQ_ENTITY_DATA_CLEANING_1@TO_PROD_ENTIDAD700(v_sche_ori, v_fecha_dep, v_proc_id);
...
BEGIN
    ...
    IF v_hora_actual < '02:59' OR v_hora_actual >= '03:10' THEN
        SOPORTEDBA.PRC_WRITE_ERROR_LOG_DATA_CLEANING(
            NULL,
            SYSDATE,
            NULL,
...
END PKG_WFGADQ_ENTITY_DATA_COPY_AND_CLEANING_700;
/
